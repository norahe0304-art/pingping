#!/usr/bin/env python3
"""Deterministically sync discord-feed shared memory into daily memory.

- Source: memory/shared/YYYY-MM-DD-discord-feed.md
- Target: memory/YYYY-MM-DD.md
- State:  memory/.discord_feed_sync_state.json (processed message ids per day)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sync discord feed into daily memory")
...
def ensure_daily_file(daily_path: Path, date_str: str) -> None:
    if daily_path.exists():
        return
    daily_path.write_text(f"# {date_str}\n\n", encoding="utf-8")


def append_summary(daily_path: Path, date_str: str, feed_path: Path, new_msgs: List[Dict], sender_name: str) -> None:
...
def main() -> int:
    ...
    daily_path = workspace / "memory" / f"{date_str}.md"
    state_path = workspace / "memory" / ".discord_feed_sync_state.json"
    ...
    ensure_daily_file(daily_path, date_str)
    append_summary(daily_path, date_str, feed_path, new_msgs, args.sender)
    ...
    if args.verbose:
        print(f"appended {len(new_msgs)} messages into {daily_path}")
